...
# JOB INIT <node> <rem time>             - The job has been starting on <node> but the process is still initializing.
# JOB RUNNING <node> <port> <rem time>   - The notebook is running on node <node> and port <port>. <rem time> is the remaining time until it will be killed.
# 
...
NOTEBOOKDOWNLOAD=http://babas.clps.brown.edu:11000/data/${PACKAGE}.tar.gz
...
if [[ "$2" == "START" ]]; then
	...
	cd "$NOTEBOOKPATH"
	wget -N -q "$NOTEBOOKDOWNLOAD" -O ${PACKAGE}.tar.gz
...
format="%i %j %L %N %S %R"
JOBSTATUS=$(squeue -h -u $USER --format="%i %j %N %L" -t R -S L | grep -m 1 $(basename "$0") | sed "s/  \+/ /g")
if [[ -z "$JOBSTATUS" ]]; then
	JOBSTATUS=$(squeue -h -u $USER --format="%i %j %R %S" -t PD -S S | grep -m 1 $(basename "$0") | sed "s/  \+/ /g")
fi
...
if [[ -z "$JOBSTATUS" ]]; then
	...
	echo JOB OFF
	mkdir -p logs
	sbatch -p bibs-gpu --gres=gpu:1 --mem=32g -t 3:00:00 -o logs/slurm-%j.out "$0" "$PACKAGE" START >/dev/null
else
	# Job is running. Extract status fields
	JOBID=$(echo $JOBSTATUS | cut -d " " -f 1)
	REMTIME=$(echo $JOBSTATUS | cut -d " " -f 4)
	NODE=$(echo $JOBSTATUS | cut -d " " -f 3)
	# Job to be killed?
	if [[ "$2" == "KILL" ]]; then
		scancel $JOBID
		echo JOB KILLED
	...
	if [[ "$NODE" =~ ^\(.*\)$ ]]; then
		# Job still pending. $REMTIME is estimated start time.
		echo JOB QUEUED $NODE $REMTIME
	else
		# Slurm job output file created?
		LOGFILE="logs/slurm-${JOBID}.out"
		if [ ! -f "$LOGFILE" ]; then
			# No output file yet.
			echo JOB INIT $NODE $REMTIME
		else
			...
			if [[ -z "$PORT" ]]; then
				...
				echo JOB RUNNING $NODE $PORT $REMTIME $NBPASS
			fi
		fi
	fi
fi
